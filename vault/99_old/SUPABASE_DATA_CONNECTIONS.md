...
- **`prediction_fixture_results`**(`20260905120000_prediction_fixture_results.sql`)는 `prediction_results`와 컬럼이 같지만 주차 정산 게이트가 없는 자매 view입니다 — 종료된 경기면 그 주가 아직 진행 중이어도 나옵니다. 랭킹(`week_leaderboard`/`season_leaderboard`)은 여전히 정산 게이트가 있는 `prediction_results` 위에만 있습니다 — 이 view는 완료 허브의 "경기 단위 미리보기" 전용이고 랭킹 집계에는 쓰이지 않습니다. 조회는 `getMyFixtureResults()`.
- **경기가 끝났지만 주차가 진행 중인 구간**에도 이제 완료 화면(`PredictionDone`)이 `prediction_fixture_results`를 읽어 결과 화면과 같은 판정 블록(`MatchResultBlock`, `PredictionResult.tsx`가 export)을 그대로 보여줍니다(2026-09-05, TEA-34) — 더는 적중 배지만 보여주지 않습니다. 다만 `fixture_player_ratings` 행 수가 아직 임계값(11, `20260905130000_prediction_results_rated_players_count.sql`의 `rated_players_count` 컬럼 + `sync-fixture-ratings`의 `MIN_RATED_PLAYERS`와 동일값 — 세 곳이 서로를 가리키는 주석으로 매직넘버 중복을 감수함) 미만이면 "내 선수 픽" 카드만 "평점 집계 중이에요"로 대체됩니다(`ratingsSettled` → `MatchResultBlock`의 `pickPointsReady`). 아직 안 끝난 경기는 킥오프 전이면 "지금 예측하기" 카드, 킥오프는 지났지만 안 끝났으면(`locked && !finished`) "결과를 기다리는 중이에요" 캡션만 보여줍니다. 참고로 `lib/predictions/result.ts`의 `matchHit()`(DB `prediction_match_points`와 같은 기준)는 결과 화면 히어로의 적중 배지에 별도로 쓰입니다.
