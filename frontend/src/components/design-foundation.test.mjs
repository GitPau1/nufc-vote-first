...
test('image banners use a readable dark overlay for white text', () => {
  ...
  const files = [
    ...
    // OverallRatingResultView도 같은 이유로 이 목록에서 빠졌다 —
  ...
  const ratingPoll = source('components/composition/polls/OverallRatingPollClient.tsx')
  assert.doesNotMatch(ratingPoll, /banner-text-overlay/)
